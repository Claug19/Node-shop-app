...
`aws ecr get-login-password \
         ...
         --username AWS \
         --password-stdin 943580395454.dkr.ecr.us-east-1.amazonaws.com`
...
do
	...
	(cd microservices/$SERVICE_NAME && npm install);
	docker build -t $SERVICE_NAME ./microservices/$SERVICE_NAME
	...
	STATUS=`aws ecs describe-microservices \
		--microservices $SERVICE_NAME \
		--region $REGION \
		--cluster "ecs-cluster-isoc-ECSCluster-XFMRZhzUk6qS" \
	...
	if [ "$STATUS" != "ACTIVE" ]; then
		...
		if [ -e "./services/$SERVICE_NAME/rule.json" ]; then
			...
			LISTENER_ARN=`aws elbv2 describe-listeners \
				--region $REGION \
				--load-balancer-arn "arn:aws:elasticloadbalancing:us-east-1:943580395454:loadbalancer/app/ecs-c-ECSAL-1PMZUDU1HUJ31/d97bb7c168757e51" \
			...
			RULE_DOC=`cat ./microservices/$SERVICE_NAME/rule.json |
			...
			RESULT=`aws ecs create-service \
				...
				--role "arn:aws:iam::943580395454:role/ecs-cluster-isoc-ECSServiceRole-1XV8MWGRS0S5W" \
